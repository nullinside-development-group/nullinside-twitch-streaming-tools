...
public class EmojiDeduplicationFilter : ITtsFilter {
  ...
  public Tuple<string, string> Filter(IConfiguration configuration, OnMessageReceivedArgs twitchInfo, string username, string currentMessage) {
    ...
    string[] messageParts = currentMessage.Split(" ", StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
    for (int i = 0; i < messageParts.Length; i++) {
      string part = messageParts[i].ToLowerInvariant().Trim();

      // Check for twitch, frankerzface, and betterttv
      bool hasEmote = emoteText.Contains(part);
      ...
      for (int x = 0; x < charArray.Length; x++) {
        ...
        char possibleEmote = charArray[x];
        if (possibleEmote > 127) {
          ...
          if (encounteredEmotes.Contains(possibleEmote.ToString())) {
            ...
            // If we haven't, add it to the list of things we've encountered for later.
